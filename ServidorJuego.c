...
void *AtenderCliente (void *socket)
{
	...
	conn = mysql_real_connect (conn, "shiva2.upc.es","root", "mysql", "M4_BBDDParchis",0, NULL, 0);
	if (conn==NULL) {
	...
	while(terminar==0)
	{
		...
		if (codigo == 1)
		{
			...
			if(n_rows == 0)
			{
				...
				strcpy(consulta, "");
				strcpy(consulta2, "");
				strcpy(consulta, "INSERT INTO usuario (Nombre_Usuario, Fecha_Registro, Correo, Contrase�a, ELO_Actual, ELO_Mas_Alto, Num_Partidas, Porcentaje_1er_Puesto, Porcentaje_2do_Puesto, Porcentaje_3er_Puesto, Porcentaje_4to_Puesto, Puntuacion_Media_Por_Partida, Racha_Victorias, Partidas_ID, Contrincantes_ID, Amigos_ID, Num_Amigo) ");
		...
		if (codigo == 3) {
			...
			snprintf(buff2, sizeof(buff2), "%d", numPartidas);
			printf("Buff2: %s\n", buff2);
		...
		if (codigo == 4) {
			...
			if (row != NULL) {
				...
				snprintf(buff2, sizeof(buff2), "%d", eloActual);
				printf("Buff2: %s\n", buff2);
		...
		if (codigo == 5) {
			...
			if (row != NULL) {
				...
				snprintf(buff2, sizeof(buff2), "%d", numAmigo);
				printf("Buff2: %s\n", buff2);
